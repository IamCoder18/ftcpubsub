...
Thanks for your interest in contributing to AaravLabs PubSub! This document
...
- `src/main/java/com/aaravlabs/pubsub/` — the library.
- `src/main/java/com/aaravlabs/pubsub/ftc/` — FTC-specific helpers
  (`SafeOpMode`, `HardwareActions`, `GamepadAdaptor`, ...).
- `src/main/java/com/aaravlabs/pubsub/annotation/` — annotation types.
- `src/main/java/com/aaravlabs/pubsub/internal/` — reflection-based binder;
  not part of the public API.
- `src/main/resources/META-INF/proguard/pubsub.pro` — R8 keep rules.
...
- `src/test/java/com/aaravlabs/pubsub/` — JUnit 5 tests.
...
- **R8 survival.** All reflection paths must keep working after `r8` minification.
  Add proguard rules to `src/main/resources/META-INF/proguard/pubsub.pro` if you
...
- The pubsub library version (`com.aaravlabs:pubsub:X.Y.Z`).
- The FTC SDK version (`RobotCore:X.Y.Z`).
- What you expected vs. what happened.

Issues go at <https://github.com/IamCoder18/ftcpubsub/issues>.
